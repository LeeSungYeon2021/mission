/**
 * 
 */

$(function() {
	dateInput();
	calendar();
	selectPlan();
	planCount();
});


var count = 1;
var date ;
var currentYear;
var currentMonth;
var currentDay;

//매달 1일 요일
var firstDay;

//매달 마지막 날짜
var lastDay;

//현재 년월일 표시
function dateInput(data, btnDate) {
	console.log('full', btnDate);
	if (data == 'P' || data == 'N') {
		date = new Date(btnDate);
	} else {
		date = new Date();
	}

	currentYear = date.getFullYear();
	currentMonth = date.getMonth() + 1;
	currentDay = date.getDate();
	console.log('일 ', currentDay);
	console.log('ㅇㅁㅅㄷ' , date);
	$("#currentYear").val(currentYear + '년');
	$("#currentMonth").val(currentMonth + '월');

}

//달력 화면 생성
function calendar() {

	$("#body_table tbody").children().remove();

	firstDay = new Date(date.getFullYear(), date.getMonth(), 1).getDay();
	lastDay = new Date(currentYear, currentMonth, 0).getDate();

	for (let i = 0; i < firstDay + lastDay; i++) {

		//매주 tr 생성
		if (i == 0 || i % 7 == 0) {
			var tr = $("<tr>");
		}
		//전체 td 생성
		let td = $("<td>");

		//td 공란채우기
		if (i < firstDay) {
			td.addClass('tdEmpty_' + i);
		} else {

			//날짜 표출  input
			let dayInput = $("<input>");
			dayInput.attr({
				'class': 'inputDay',
				'type': 'text',
				'value': count,
				/*'readonly': 'true'*/
			});

			if (i == 0 || i % 7 == 0) {
				dayInput.css('color', 'red');
			}
			
			//수정해야함
			if (i == 6 || i == 13 || i == 20 || i == 27 || i==34 )  {
				dayInput.css('color', 'blue');
			}	
			
			let dayHidden;

			if (count < 10) {
				dayHidden = $("<input>");
				dayHidden.attr({
					'type': 'hidden',
					'value': currentYear + '-0' + currentMonth + '-0' + count,
					'name': 'fullDay',
					'class': 'hiddenDay_' + count
				})
			} else {
				dayHidden = $("<input>");
				dayHidden.attr({
					'type': 'hidden',
					'value': currentYear + '-0' + currentMonth + '-' + count,
					'name': 'fullDay',
					'class': 'hiddenDay_' + count
				})
			}
			//전체 일정 개수 표출 input
			let planCountInput = $("<input>");
			planCountInput.attr({
				'class': 'planCountInput',
				'type': 'text',				
				'readonly': 'true',				
			}).css({
				'text-align': 'right',
				'font-size' : '10px'	
			});

			let ul = $("<ul>");
			ul.addClass('list-group');
			td.addClass('tdDay_' + count)
			td.append(dayInput);
			td.append(dayHidden);
			td.append(planCountInput);
			td.append(ul);
			count++;


		}
		if (count > lastDay) {
			count = 1;
		}
		tr.append(td);
		$("#body_table tbody").append(tr);	
	}

}

//달별,일별 일정 개수 조회
function planCount() {
	var countDate = currentYear + '/' + '0' + currentMonth;
	
	$.ajax({
		url: '/plan_count',
		datatype: 'json',
		method: 'post',
		data: {day : countDate},
		success: function(data) {
			console.log('카운트데이터',data[0]);
			
			$("#plan_countAll").html('전체일정 (' + data[0].monthCount + '개)');
			console.log('자름',(data[0].plan_startDate).substr(0,10));
			for(let i=0;i<data.length ;i++) {
				let subDay = data[i].plan_startDate.substr(0,10);
				
				$(".planCountInput").each(function(idx) {
					var self = $(this);

					var hiddenVal = $(this).prev().val();

					if (hiddenVal === subDay) {

						self.val(data[i].dayCount + '개');
					}
				})
				
			}
}

});
}
//현재 월 전체 일정 조회
function selectPlan() {
		
	var dayData = currentYear + '/' + '0' + currentMonth;		

	$.ajax({
		url: '/plan_select',
		datatype: 'json',
		method: 'post',
		data: {day : dayData},
		success: function(data) {

			console.log(data);			
			
			for (let i = 0; i < data.length; i++) {

				let full = data[i].plan_startDate;
				let year = full.substring(0, 4);
				
				let month = full.substring(6, 7);
				let days = full.substring(8, 10);
				let dayStr = year + '-0' + month + '-' + days;
				let str = "Y";
				
				$("input[name=fullDay]").each(function(idx) {
					var self = $(this);

					var hiddenVal = $(this).val();

					if (hiddenVal === dayStr) {
						
						var li = $("<li>");

						if ((data[i].plan_select).trim() === 'Y') {
							li.addClass('list-group-item list-group-item-danger');

						} else {
							li.addClass('list-group-item');
						}
						
						li.append(data[i].plan_title);
											
						self.next().next().append(li);
					}
				})
			}

		}
	});

}

//이전달 버튼
$("#prevBtn").on('click', function() {

	if (currentMonth == 1) {
		currentMonth = 13;
		currentYear -= 1;
	}
	let fullDate = currentYear + ',' + (currentMonth - 1);


	dateInput('P', fullDate);
	calendar();
	selectPlan();
});

//다음달 버튼
$("#nextBtn").on('click', function() {

	if (currentMonth == 12) {
		currentMonth = 0;
		currentYear += 1;
	}
	let fullDate = currentYear + ',' + (currentMonth + 1);


	dateInput('P', fullDate);
	calendar();
	selectPlan();
});

//일정등록 modal
$(document).on('click', '.inputDay', function(e) {
	$("#plan-title").val('');
	$("#plan-content").val('');
	$(".modal").modal('show');
	var target = $(e.target);
	
	//현재 날짜 입력
	$("#plan_startDate").val(target.next().val());
});

$("#plan_startDate").on('change',function(e) {
	let tt = $(e.target).val();
	let ttt = $(".hiddenDay_"+currentDay).val();	
})



